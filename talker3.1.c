...
struct sys_info_type     
{
    ...
    double free_mem;
    double proportional_used_memory;
    int machine_type;
    long disk_activity;
...
static void initialise_sys_info(struct sys_info_type *sys_info) 
{
    ...
    sys_info->proportional_used_memory = 0.0;
...
static void find_free_memory(struct sys_info_type *sys_info)
{
	...
	while(fgets(line,256, fp))
	 ...
	 sys_info->proportional_used_memory = ((1.0 - (actual_mem_free / mem_total)) * 100);
...
{ 
    ...
    while(1)
    {
        ...
        printf("\nProportional free memory on this machine is: %lf %%", sys_info.proportional_used_memory);
